...
release-aarch64: update
	CROSS_ENV_PASSTHROUGH="CFLAGS" CFLAGS="-w" cross build --target aarch64-unknown-linux-gnu --release --all-features
...
release-aarch64-static: update
	CROSS_ENV_PASSTHROUGH="CFLAGS" CFLAGS="-w" cross build --target aarch64-unknown-linux-musl --release --all-features
